...
object MapperRules {
   ...
   *
   * To get snake_case, add
   *
   * MapperRules.columnName = ClassHelpers.unCamelCase
   */
  var columnName: String => String = name => name.toLowerCase
   ...
   *
   * To get snake_case, add
   * MapperRules.tableName = ClassHelpers.unCamelCase
   */
  var tableName: String => String = name => name.toLowerCase
}
...
trait MetaMapper[A<:Mapper[A]] extends BaseMetaMapper with Mapper[A] {
  ...
  private def fixTableName(name: String) = {
    val tableName = MapperRules.tableName(clean(name))
